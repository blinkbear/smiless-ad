...
class PathSearchNoCuda:
    ...
    def get_running_plan_df(self):
        ...
        self.A_star(self.df, self.SLA)
        if self.available_solution is None:
            print("No feasible solution")
            self.available_solution = self.df
            self.available_solution["device"] = "cpu"
            self.available_solution["prewarm_window"] = 0
    ...
    def calc_task_cost(self, row):
        ...
        """
        cost = 0
        if row["cpu_running_time"] < self.IT and self.IT < row["cpu_execution_time"]:
    ...
    def calc_prewarm_window(self):
        ...
        self.available_solution["prewarm_window"] = np.where(cpu_prewarm)
        return
    ...
    def calc_heuristic_cost(self, df, index, SLA):
        ...
        """
        current_time = df.loc[index - 1, "current_running_time"] if index > 0 else 0
        if current_time > SLA:
            return float("inf")
        estimation_minimum_execution_time = (
            df[["cpu_inference_time"]][index:].min(axis=1).sum()
        )
    ...
    def A_star(self, df, SLA):
        """
        Runs A* search algorithm to find the optimal solution to a scheduling problem.

        Args:
            df (pandas.DataFrame): a DataFrame containing tasks to be scheduled
            SLA (int): the maximum allowed completion time for the schedule
