...
class KubeOperator:
    ...
    def check_scaler_ready(self, function_name, warm):
        while True:
            status = []
            ...
            if len(status) > 0 and warm == "cold":
                sleep(1)
            elif len(status) == 0 and warm == "warm":
                sleep(1)
            else:
                break
    ...
    def create_or_update_workflow_configmap(
        self, workflow_name, workflow_annotation, data
    ):
        configmap = client.V1ConfigMap()
        configmap.metadata = client.V1ObjectMeta(
            name=workflow_name, annotations=workflow_annotation
        )
        configmap.data = data
        if self.check_workflow_configmap(workflow_name):
            self.api.patch_namespaced_config_map(
                workflow_name, self.namespace, configmap
            )
        else:
    ...
    def check_workflow_configmap_ready(self, workflow_name):
        while True:
            ...
            for i in ret.items:
                if i.metadata.name == workflow_name:
                    if (
                        "dag_parse_finished" in i.metadata.annotations
                        and i.metadata.annotations["dag_parse_finished"] == "true"
                    ):
                        return True
